...
@interface MASPhotoBrowser () <MASPhotoViewDelegate> {
    // 滚动的view
...
@implementation MASPhotoBrowser
...
- (void)createToolbar {
    ...
    _toolbar = [[MASPhotoToolbar alloc] init];
...
- (void)setPhotos:(NSArray *)photos {
    ...
    for (NSInteger i = 0; i<_photos.count; i++) {
       // MJPhoto *photo = _photos[i];
        ((MASPhoto*)_photos[i]).index = i;
        ((MASPhoto*)_photos[i]).firstShow = i == _currentPhotoIndex;
    }
}
...
- (void)setCurrentPhotoIndex:(NSUInteger)currentPhotoIndex {
    ...
    for (int i = 0; i<_photos.count; i++) {
       // MJPhoto *photo = _photos[i];
        ((MASPhoto*)_photos[i]).firstShow = i == currentPhotoIndex;
    }
...
- (void)showPhotos {
	...
	for (MASPhotoView *photoView in _visiblePhotoViews) {
...
- (void)showPhotoViewAtIndex:(NSInteger)index {
    ...
    MASPhotoView *photoView = [self dequeueReusablePhotoView];
    if (!photoView) { // 添加新的图片view
        photoView = [[MASPhotoView alloc] init];
...
- (void)loadImageNearIndex:(NSInteger)index {
    ...
    if (index > 0) {
        ...
        MASPhoto *photo = _photos[index - 1];
        //[SDWebImageManager downloadWithURL:photo.url];
        
        [YYWebImageManager downloadWithURL:photo.url];
    }
...
- (MASPhotoView *)dequeueReusablePhotoView
{
    MASPhotoView *photoView = [_reusablePhotoViews anyObject];
	if (photoView) {
